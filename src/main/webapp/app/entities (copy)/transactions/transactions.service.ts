...
export class TransactionsService {
  ...
  protected convertDateFromClient(transactions: ITransactions): ITransactions {
    const copy: ITransactions = Object.assign({}, transactions, {
      txnPostDt: transactions.txnPostDt && transactions.txnPostDt.isValid() ? transactions.txnPostDt.format(DATE_FORMAT) : undefined
    });
    return copy;
  }
  ...
  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.txnPostDt = res.body.txnPostDt ? moment(res.body.txnPostDt) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((transactions: ITransactions) => {
        transactions.txnPostDt = transactions.txnPostDt ? moment(transactions.txnPostDt) : undefined;
      });
    }
    return res;
  }
