...
export class LookupsComponent implements OnInit, OnDestroy {
  ...
  lookupTypes1: ILookupTypes[] = [];
  lookupTypeSelected: any=[];
  lookups?: ILookups[];
  eventSubscriber?: Subscription;

  constructor(protected activatedRoute: ActivatedRoute, protected lookupsService: LookupsService, protected eventManager: JhiEventManager, protected modalService: NgbModal, protected lookupTypesService: LookupTypesService) {}

  loadAll(): void {
    this.activatedRoute.data.subscribe(({ lookupTypes }) => {(this.lookupTypes1 = lookupTypes);
      this.lookupsService.lookupsById(this.lookupTypes1['id']).subscribe((res: HttpResponse<ILookups[]>) => (this.lookups = res.body || []));
    });
