package com.jhipster.reval.ui.web.rest;

import com.jhipster.reval.ui.domain.LookupTypes;
import com.jhipster.reval.ui.service.LookupTypesService;
import com.jhipster.reval.ui.web.rest.errors.BadRequestAlertException;
import com.jhipster.reval.ui.service.dto.LookupTypesCriteria;
import com.jhipster.reval.ui.service.LookupTypesQueryService;

import io.github.jhipster.web.util.HeaderUtil;
import io.github.jhipster.web.util.ResponseUtil;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;
import org.springframework.beans.factory.annotation.Value;
import org.springframework.http.ResponseEntity;
import org.springframework.web.bind.annotation.*;

import javax.validation.Valid;
import java.net.URI;
import java.net.URISyntaxException;
import java.util.List;
import java.util.Optional;

/**
 * REST controller for managing {@link com.jhipster.reval.ui.domain.LookupTypes}.
 */
@RestController
@RequestMapping("/api")
public class LookupTypesResource {

    private final Logger log = LoggerFactory.getLogger(LookupTypesResource.class);

    private static final String ENTITY_NAME = "lookupTypes";

    @Value("${jhipster.clientApp.name}")
    private String applicationName;

    private final LookupTypesService lookupTypesService;

    private final LookupTypesQueryService lookupTypesQueryService;

    public LookupTypesResource(LookupTypesService lookupTypesService, LookupTypesQueryService lookupTypesQueryService) {
        this.lookupTypesService = lookupTypesService;
        this.lookupTypesQueryService = lookupTypesQueryService;
    }

    /**
     * {@code POST  /lookup-types} : Create a new lookupTypes.
     *
     * @param lookupTypes the lookupTypes to create.
     * @return the {@link ResponseEntity} with status {@code 201 (Created)} and with body the new lookupTypes, or with status {@code 400 (Bad Request)} if the lookupTypes has already an ID.
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PostMapping("/lookup-types")
    public ResponseEntity<LookupTypes> createLookupTypes(@Valid @RequestBody LookupTypes lookupTypes) throws URISyntaxException {
        log.debug("REST request to save LookupTypes : {}", lookupTypes);
        if (lookupTypes.getId() != null) {
            throw new BadRequestAlertException("A new lookupTypes cannot already have an ID", ENTITY_NAME, "idexists");
        }
        LookupTypes result = lookupTypesService.save(lookupTypes);
        return ResponseEntity.created(new URI("/api/lookup-types/" + result.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, true, ENTITY_NAME, result.getId().toString()))
            .body(result);
    }

    /**
     * {@code PUT  /lookup-types} : Updates an existing lookupTypes.
     *
     * @param lookupTypes the lookupTypes to update.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated lookupTypes,
     * or with status {@code 400 (Bad Request)} if the lookupTypes is not valid,
     * or with status {@code 500 (Internal Server Error)} if the lookupTypes couldn't be updated.
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PutMapping("/lookup-types")
    public ResponseEntity<LookupTypes> updateLookupTypes(@Valid @RequestBody LookupTypes lookupTypes) throws URISyntaxException {
        log.debug("REST request to update LookupTypes : {}", lookupTypes);
        if (lookupTypes.getId() == null) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }
        LookupTypes result = lookupTypesService.save(lookupTypes);
        return ResponseEntity.ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, lookupTypes.getId().toString()))
            .body(result);
    }

    /**
     * {@code GET  /lookup-types} : get all the lookupTypes.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of lookupTypes in body.
     */
    @GetMapping("/lookup-types")
    public ResponseEntity<List<LookupTypes>> getAllLookupTypes(LookupTypesCriteria criteria) {
        log.debug("REST request to get LookupTypes by criteria: {}", criteria);
        List<LookupTypes> entityList = lookupTypesQueryService.findByCriteria(criteria);
        return ResponseEntity.ok().body(entityList);
    }

    /**
     * {@code GET  /lookup-types/count} : count all the lookupTypes.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/lookup-types/count")
    public ResponseEntity<Long> countLookupTypes(LookupTypesCriteria criteria) {
        log.debug("REST request to count LookupTypes by criteria: {}", criteria);
        return ResponseEntity.ok().body(lookupTypesQueryService.countByCriteria(criteria));
    }

    /**
     * {@code GET  /lookup-types/:id} : get the "id" lookupTypes.
     *
     * @param id the id of the lookupTypes to retrieve.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the lookupTypes, or with status {@code 404 (Not Found)}.
     */
    @GetMapping("/lookup-types/{id}")
    public ResponseEntity<LookupTypes> getLookupTypes(@PathVariable Long id) {
        log.debug("REST request to get LookupTypes : {}", id);
        Optional<LookupTypes> lookupTypes = lookupTypesService.findOne(id);
        return ResponseUtil.wrapOrNotFound(lookupTypes);
    }

    /**
     * {@code DELETE  /lookup-types/:id} : delete the "id" lookupTypes.
     *
     * @param id the id of the lookupTypes to delete.
     * @return the {@link ResponseEntity} with status {@code 204 (NO_CONTENT)}.
     */
    @DeleteMapping("/lookup-types/{id}")
    public ResponseEntity<Void> deleteLookupTypes(@PathVariable Long id) {
        log.debug("REST request to delete LookupTypes : {}", id);
        lookupTypesService.delete(id);
        return ResponseEntity.noContent().headers(HeaderUtil.createEntityDeletionAlert(applicationName, true, ENTITY_NAME, id.toString())).build();
    }
}
